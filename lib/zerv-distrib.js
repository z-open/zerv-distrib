...
if (zervDistrib) {
  ...
  async function formatClusterStatus(findTenandByIdFn) {
    ...
    const processes = _.slice(
        _.orderBy(await processService.findAll(), p => !p.timestamp.lastModifiedDate ? - Date.now() : -(new Date(p.timestamp.lastModifiedDate).getTime())),
        0,
        500
    );
    ...
    const statusTb = (await Promise.all(_.map(statuses, formatServerStatus))).join('');
    ...
    <table border="1" cellpadding="7" width="100%">${statusTb}</table>
    <h2>CLUSTER PROCESS QUEUE</h2>
    <table border="1" cellpadding="7"  width="100%">${queueTb}</table>
    </body>
    </html>`);
    ...
    async function formatServerStatus(s) {
      ...
      // it indicates that the user has disconnected but not that it logged out.
      // if disconnected, the socket might reconnect anytime. it helps knowing
      // when the user logged in but it did not officially logout
