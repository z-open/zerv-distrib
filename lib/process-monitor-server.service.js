...
// this constant defines the maximum time in seconds before a server is 
// no longer considered as alive
//
// IMPORTANT:
// Currently algo restarts process that are not handled by a alive server.
// this will be changed in the future
// There is no much value to do this. it will be better to just make them error out.
const DEFAULT_SERVER_STAY_ALIVE = 600;
...
function monitorQueue(serverName, port, capacityProfile) {
  ...
  serverStatusService.createOne(serverName, process.env.ZERV_STAY_ALIVE || DEFAULT_SERVER_STAY_ALIVE);
...
function checkIfProcessIsStalled(serverId, process) {
  ...
  if (!serverOwner) {
    // if the local server just started, it will might not have received the serverOwner status, expecially if the server owner is too busy to respond
    // This could lead to restart the process too early. let's wait arbitrary 4 cycles to give opportunities to other servers to notify their presence.
    if (moment.duration(moment().diff(thisServerStart)).asSeconds() > (process.env.ZERV_STAY_ALIVE || DEFAULT_SERVER_STAY_ALIVE) * 4) {
