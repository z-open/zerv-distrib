...
async function getLock() {
  ...
  if (!processQueueResisLock) {
    ...
    const client = new IoRedis(connectionParams);
    // --------------------------
    processQueueResisLock = ioRedisLock.createLock(client, {
      timeout: process.env.REDIS_LOCK_TIMEOUT_IN_MS || 20000,
      retries: process.env.REDIS_LOCK_RETRY || 3,
  ...
  try {
    ...
    logger.error('Redis Locking error', err);
        ...
        .execute(async (transaction) => {
          ...
          for (const process of queue) {
            ...
            if (process.status === processService.PROCESS_STATUSES.PENDING || checkIfProcessIsStalled(serverId, process)) {
              if (canServerExecuteProcessNow(process)) {
