...
let zerv;
let activeProcesses = {};
let waitForProcessingQueue, capacityProfile;
...
function listenProcessQueueForNewRequests() {
  ...
  zerv.onChanges('TENANT_PROCESS_DATA', async (tenantId, process, notificationType) => {
    if (process.status === processService.PROCESS_STATUSES.PENDING) {
      ...
      scheduleToCheckForNewProcessResquests();
    }
  });
 ...
 *
 */
function scheduleToCheckForNewProcessResquests() {
  ...
  try {
    ...
    for (const process of queue) {
      ...
      if (process.status === processService.PROCESS_STATUSES.PENDING) {
        ...
        {
          try {
            checkIfProcessIsNotStalled(process);
          } catch (err) {
 ...
 */
function checkIfProcessIsNotStalled(process) {
 ...
 * @param {TenantProcess} process
 * @param {String} byServer is the serverId
 */
async function _executeProcess(process, byServer) {
  ...
  return await processService.updateOne(process.tenantId, process);
